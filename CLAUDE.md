...
Браузерная экономическая стратегия про FMCG-рынок: три роли — ритейлер,
производитель, дистрибьютор/логист. Ранняя альфа / вертикальный срез.

## 🎯 Северная звезда (главная задача)
**Сделать интересную игру про цепочку поставок.** Игрок строит и оптимизирует цепочку
сырьё → производство → логистика → полка → покупатель и побеждает **умных ботов**.
- **Вектор:** одиночка против умных ботов — приоритет; мультиплеер допиливаем позже.
- **Глубина:** сбалансированная середина (реализм, но читаемо, с подсказками).
- **Четыре кайфа:** логистика/оптимизация · ценовые войны · торговля/переговоры · рост империи.
- Дизайн-правда: `docs/TECHNICAL_SPEC.md`. План: `docs/ROADMAP.md`. Аудит: `docs/AUDIT.md`.
  Исходное широкое MMO-видение в `docs/HANDOFF.md` — «куда можно вырасти», НЕ текущая цель.
- **Чек-лист каждой фичи:** служит цепочке? даёт интересное решение (а не просто реализм)?
  читаемо игроку? делает ботов/мир умнее? — иначе отложить.
- ⏸️ Сейчас НЕ делаем: нормализацию PostgreSQL, транзакционный день, realtime-мультиплеер,
  чат, админку баланса — они для мультиплеера, не для «весёлой» одиночки.
