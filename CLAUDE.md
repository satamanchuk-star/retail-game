...
## Git
Репозиторий: `satamanchuk-star/retail-game`, дефолтная ветка `main`. После завершённой итерации **коммитить и сразу пушить на origin** без отдельного запроса. На `main` сначала создавать рабочую ветку; пушить в неё (`git push -u origin <branch>`), а не в `main` напрямую.
